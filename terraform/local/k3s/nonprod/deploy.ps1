# K3s Non-Production Environment Deployment Script (Terraform)
# This script deploys both the cluster and metrics server stacks

Write-Host "🚀 Starting K3s Non-Production Environment Deployment (Terraform)" -ForegroundColor Green
...
$metricsSuccess = $true
$metricsSuccess = $metricsSuccess -and (Invoke-Command "terraform init" "metrics-server" "Initializing metrics server Terraform")
...
Write-Host "=" * 70 -ForegroundColor Cyan
Write-Host "✅ K3s Non-Production Environment deployed successfully!" -ForegroundColor Green
